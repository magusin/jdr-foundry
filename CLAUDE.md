...
- A `.claudeignore` exists (`*.db`, `assets/`, `GUIDE_TEST.md`, `HANDOFF.md`) but does **not** currently filter Read/Glob/Grep in this environment — verified empirically (2026-07-28): both tools still returned/matched ignored paths. Don't rely on it to save context/resources until this is reconfirmed working; the NDJSON edit trick above works regardless, via Bash.
