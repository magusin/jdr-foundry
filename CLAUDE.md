...
- `.claudeignore` excludes `*.db`, `assets/`, `GUIDE_TEST.md`, and `HANDOFF.md` from Read/Glob/Grep — the NDJSON edit trick above still works since it goes through Bash (`python3 -c "..."`, `cat`), which isn't affected by `.claudeignore`.
