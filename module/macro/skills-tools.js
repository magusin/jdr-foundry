...
(async () => {
  ...
  // ── Mêmes formules EXACTES que character-sheet-v2.js (à garder synchronisées) ──
  function skillXpToNext(level) {
    return 100 + 50 * Math.max(0, Number(level) || 0);
  }
  ...
  async function addXp(actor, skillKey, amount) {
  ...
  async function removeXp(actor, skillKey, amount) {
    const skills = foundry.utils.deepClone(actor.system?.skills ?? {});
    const s = skills[skillKey];
    if (!s) return;
    ...
    while (sub > 0) {
      ...
      if (curXp >= sub) { s.xp = curXp - sub; sub = 0; break; }
      sub -= curXp;
      s.xp = 0;
      const lvl = Number(s.level) || 0;
      if (lvl <= 0) { sub = 0; break; }
      s.level = lvl - 1;
      s.xp = Math.max(0, skillXpToNext(s.level) - 1);
    }
    skills[skillKey] = s;
    await actor.update({ "system.skills": skills });
  }
